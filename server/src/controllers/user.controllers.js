...
const generateEmailVerification = asynchandler(async (req, res) => {
    const { fullname, username, password, institution, department, age, gender, email } = req.body

    //All field check

    if (
        [fullname, username, password, institution, department, age, gender, email].some((field) => field?.trim() === "")
    ) {
    ...
    if (
        [fullname, institution, department, gender].some((field) => validateString(field) != true)
    ) {
        return res.status(400).json(ApiError(400, "Something is invalid!"))
    }

    const validateAge = validateNumber(age)

    if (!validateAge) { return res.status(400).json(ApiError(400, "Age is invalid!", validateAge)) }
    ...
    if (existedUser) { return res.status(400).json(ApiError(400, "Email or username is already exists!")) }
    ...
    const emailSent = mailSend(email, `email-verification/${verifyEmailToken}`)
...
const testingSet = asynchandler(async(req, res)=>{
    return res.cookie("name", "Dipankar").status(200).json("set")
})

const testingGet = asynchandler(async(req, res)=>{
    res.status(200).json(`cookie: ${req.cookies.name}`)
})

export { generateEmailVerification, verifyEmail, login, getUser, logout, testingGet, testingSet }